<?php
// app/Http/Requests/Back/Catalog/UpdateManufacturerRequest.php
...
class UpdateManufacturerRequest extends FormRequest
{
    ...
    {
        $routeParam = $this->route('manufacturer');
        $id         = is_object($routeParam) ? $routeParam->getKey() : (int) $routeParam;
        ...
        foreach (config('app.locales') as $code => $label) {
            ...
            $rules["slug.$lang"]             = ['nullable', 'string', 'max:255',
                                                Rule::unique('manufacturer_translations', 'slug')
                                                    ->where('locale', $lang)
                                                    ->ignore($id, 'manufacturer_id'),
            ];
