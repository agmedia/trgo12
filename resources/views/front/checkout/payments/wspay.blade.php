<form name="pay" class="needs-validation w-100" action="{{ $data['action'] }}" method="POST">
    ...
    <div class="form-check form-check-inline">
        <label class="form-check-label" for="terms">
            {!! __('front/cart.slazem_se_sa') . ' ' . __(' :terms_of_service', [
    ...
    <div class="d-flex mt-3">
        <div class="w-50 pe-3">
            <a class="btn btn-outline-primary d-block w-100" href="{{ route('naplata') }}">
                <i class="ci-arrow-left me-1"></i>
                <span class="d-none d-sm-inline">{{ __('front/cart.povratak_na_placanje') }}</span>
                <span class="d-inline d-sm-none">{{ __('front/cart.povratak') }}</span>
            </a>
        </div>
        <div class="w-50 ps-2">
            <button class="btn btn-primary d-block w-100" type="submit">
                <span class="d-none d-sm-inline">{{ __('front/cart.dovrsi_kupnju') }}</span>
                <span class="d-inline d-sm-none">{{ __('front/cart.dovrsi_kupnju') }}</span>
