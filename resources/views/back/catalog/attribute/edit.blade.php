...
@section('content')
    <form method="post" action="{{ $attribute->exists ? route('catalog.options.update', $attribute) : route('catalog.options.store') }}">
        @csrf
        @if($attribute->exists) @method('PATCH') @endif

        <div class="row g-4">
            <div class="col-lg-7">
                <div class="card">
                    <div class="card-header"><h6 class="mb-0">Attribute</h6></div>
                    <div class="card-body">
                                ...
                                <div class="tab-pane fade @if($loop->first) show active @endif" id="tab-{{ $code }}" role="tabpanel">
                                    <div class="mb-3">
                                        ...
                                        <input id="title-{{ $code }}" type="text" name="title[{{ $code }}]" class="form-control"
                                               value="{{ old('title.'.$code, optional($attribute->translation($code))->title) }}">
                                    </div>
                                    <div class="mb-3">
                                        <label class="form-label" for="slug-{{ $code }}">Slug</label>
                                        <input id="slug-{{ $code }}" type="text" name="slug[{{ $code }}]" class="form-control"
                                               value="{{ old('slug.'.$code, optional($attribute->translation($code))->slug) }}">
                                    </div>
                                </div>
                            @endforeach
                        </div>
                        ...
                        <div class="row g-3 mt-2">
                            <div class="col-sm-6">
                                ...
                                <input name="sort_order" type="number" min="0" class="form-control" value="{{ old('sort_order', $attribute->sort_order) }}">
                            </div>
                            <div class="col-sm-6 d-flex align-items-end">
                                <div class="form-check form-switch">
                                    <input class="form-check-input" type="checkbox" name="status" value="1" @checked(old('status', $attribute->status))>
            ...
            <div class="col-12">
                <div class="d-grid gap-2">
                    <button class="btn btn-primary" type="submit">{{ $attribute->exists ? 'Save changes' : 'Create option' }}</button>
